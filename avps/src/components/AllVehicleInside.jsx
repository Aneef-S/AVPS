...
const VehicleDetails = (props) => {
  const { getEntryDetailsWithRegistrations } = useFirebase();
  const [insideVehicleData, setInsideVehicleData] = useState([]);

  useEffect(() => {
    const fetchVehicleData = async () => {
      const data = await getEntryDetailsWithRegistrations();
      // Assuming entryTime and exitTime are Date objects, format them to strings
      ...
      <div className="flex-grow justify-center items-center overflow-x-auto rounded-xl">
        <table className="min-w-full bg-gray-800 border border-gray-900 ">
          ...
          <tbody className="text-gray-50 text-lg font-light">
            {insideVehicleData.map((vehicle, index) => (
